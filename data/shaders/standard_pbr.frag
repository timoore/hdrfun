...
#define VSG_LINEAR_PIPELINE 1

#ifdef VSG_LINEAR_PIPELINE
vec4 SRGBTexture(vec4 srgbIn)
{
  return srgbIn;
}

vec4 SRGBOutput(vec4 srgbIn)
{
  return srgbIn;
}
#else
...
#ifdef VSG_EMISSIVE_MAP
    vec3 emissive = SRGBTexture(texture(emissiveMap, texCoord0)).rgb * pbr.emissiveFactor.rgb;
#else
...
void main()
{
    ...
    #else
        baseColor = vertexColor * SRGBTexture(texture(diffuseMap, texCoord0)) * pbr.baseColorFactor;
    #endif
#else
    ...
    #ifdef VSG_DIFFUSE_MAP
        vec4 diffuse = SRGBTexture(texture(diffuseMap, texCoord0));
    #else
        vec4 diffuse = vec4(1.0);
    #endif

    #ifdef VSG_SPECULAR_MAP
        vec3 specular = SRGBTexture(texture(specularMap, texCoord0)).rgb;
    ...
    outColor = SRGBOutput(vec4(color, baseColor.a));
}
